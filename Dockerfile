...
# 下载需要的软件包资源。可使用 不校验、签名校验、SHA256 校验 三种方式
	DIST_NAME="${APP_NAME}-${APP_VERSION}.${dpkgOsName}-${dpkgOsArch}.tar.gz"; \
	DIST_SHA256="4b6d6ce3670b281919dac8da4bf6d644bc8403ceae215e4fd10db0f2d1e5718e"; \
	DIST_URLS=" \
		${local_url} \
...
# 二进制解压安装，并将原始配置文件拷贝至 ${APP_DEF_DIR} 中
	...
	mv ${APP_DEF_DIR}/sample.ini ${APP_DEF_DIR}/grafana.ini; \
	rm -rf "${DIST_NAME}"; \
	\
	\
	\
...
CMD ["${APP_EXEC}", "--homepath=${APP_BASE_DIR} --config=${APP_CONF_FILE}"]
